...
from realtime_radar import get_latest_scan
import threading
from RAPdata import get_RAP_data_realtime
from metpy.interpolate import interpolate_to_points
#from zdr_col_section import zdrcol

def multi_case_algorithm_ML1_realtime(REFlev, REFlev1, big_storm, storm_to_track, calibration, station, Bunkers_m, track_dis=10, GR_mins=1.0):
    #Set storm motion, if supplied
    Bunkers_m = Bunkers_m
    ...
    g = Geod(ellps='sphere')
    #Open the placefile
    f = open("REALTIME"+station+str(dt.year)+str(dt.month)+str(dt.day)+str(dt.hour)+str(dt.minute)+"_Placefile.txt", "w+")
...
#             if ((np.mean(radar4.elevation['data']) < .65) and (np.max(np.asarray(radar4.fields['differential_reflectivity']['data'])) != np.min(np.asarray(radar4.fields['differential_reflectivity']['data'])))):
    ...
    #     print(time1)
        ...
        if goodscan_flag == 1:
            ...
            if f_l == 0:
                ffd_lons, ffd_lats, ffd_Rangles = get_RAP_data_realtime(cenlon, cenlat)
...
#                             p_hail_areas.append((0))
            ...
            #One finel step: get observed separation angles using the storm motion directions
            ...
            new_cells = pd.DataFrame({
                ...
                'storm_speed' : storm_speeds,
                'storm_direction' : storm_dirs,
                ...
                'kdp_zdr_angle_Bunkers' : b_sp_angle,
                'kdp_zdr_angle_Obs' : o_sp_angle,
                'times' : storm_times
            })
            ...
            alt_levs = [1000, 2000]
            plt.savefig('Machine_Learning/ARCALG_dev'+station+str(time_start.year)+str(time_start.month)+str(day)+str(hour)+str(minute)+'.png')
            print('Figure Saved')
            plt.close()
